...
# RegionalDiffusionXLPipeline.py
...
def GPT4(prompt, key):
    """
    使用 OpenAI API 生成文本，并从中提取分割比例和区域提示。

    Args:
        prompt (str): 用户输入的提示词。
        key (str): OpenAI API 密钥。
        sk-3e15c18401684825921793fc26b67916

    Returns:
        dict: 包含 "Final split ratio" 和 "Regional Prompt" 的字典。
    """

    client = OpenAI(api_key=key, base_url='https://api.deepseek.com') # 使用 openai 库初始化 OpenAI 客户端
    # 从文件中读取模板
    try:
        with open('/content/template.txt', 'r', encoding='utf-8') as f:
            template=f.readlines()
    except FileNotFoundError:
        print("错误：找不到 template.txt 文件。")
        return None # 或抛出异常

    # 构建完整的提示词
    user_textprompt=f"Caption:{prompt} \n Let's think step by step:"
    textprompt= f"{''.join(template)} \n {user_textprompt}" # ''join() 避免多余空格
    ...
    try:
        ...
        chat_completion = client.chat.completions.create(
            ...
            # max_tokens=1024, # 可以限制生成 token 数量 (可选)
            # temperature=0.7, # 可以调整生成多样性 (可选)
        )
...
def get_params_dict(output_text):
    """
    从 deepseek 生成的文本中提取分割比例和区域提示。

    Args:
        output_text (str): deepseek 生成的文本。
...
pipe = RegionalDiffusionXLPipeline.from_pretrained("cagliostrolab/animagine-xl-4.0",torch_dtype=torch.float16, use_safetensors=True)
# pipe = RegionalDiffusionXLPipeline.from_pretrained("./animagine-pony-attention-merged-xl",torch_dtype=torch.float16, use_safetensors=True)
